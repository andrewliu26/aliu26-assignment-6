...
def generate_plots(N, mu, sigma2, S):
    ...
    slope = model.coef_[0]  # Extract the slope from the fitted model
    ...
    for _ in range(S):
        ...
        X_sim = np.random.rand(N)  # Generate random values for X_sim between 0 and 1

        # TODO: Generate Y values with normal additive error (mean mu, variance sigma^2)
        Y_sim = X_sim + np.random.normal(
            mu, np.sqrt(sigma2), N
        )  # Generate Y_sim with normal additive error

        # TODO: Fit a linear regression model to X_sim and Y_sim
        sim_model = LinearRegression()  # Initialize model
        sim_model.fit(X_sim.reshape(-1, 1), Y_sim)  # Fit the model to X_sim and Y_sim

        # TODO: Append the slope and intercept of the model to slopes and intercepts lists
        slopes.append(sim_model.coef_[0])  # Append slope to slopes list
        intercepts.append(sim_model.intercept_)  # Append intercept to intercepts list
    ...
    intercept_more_extreme = (
        sum(i < intercept for i in intercepts) / S
    )  # Already provided

    return "static/plot1.png", plot2_path, slope_more_extreme, intercept_more_extreme
